# Agent Teams Builder 1.0.3 測試報告
...
- macOS arm64 實機一鍵安裝：Node v22.23.2、Codex CLI 0.148.0、Claude Code 2.1.270；兩個宿主均安裝並啟用 1.0.3。
...
- Claude Code 的本機 OAuth 已過期，因此模型對話 E2E 回傳 `Failed to authenticate: OAuth session expired and could not be refreshed`；Plugin 本身在 `claude plugin list` 顯示 1.0.3、enabled，manifest、Skills、Hook 與 MCP 組件可載入。
- Windows 實機結果以 GitHub Actions 的 `windows-latest` 工作為準；工作流會安裝真實 Codex／Claude Code CLI 並執行 Windows PowerShell 一鍵安裝器。Release 前須確認該工作為綠色。
