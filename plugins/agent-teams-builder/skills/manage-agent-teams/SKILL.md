...
- 開啟視覺管理頁：呼叫 `dashboard_open`。頁面會自動更新 Agents、Skills、Workflows、Play 執行記錄與每日排程。
- 建立／修改：切換到 `build-agent` 流程，先預覽、再由使用者 Double Check、最後提交。
- 執行：切換到 `run-agent` 流程。
- Agent 檔案預設位於 macOS／Windows 使用者的 `Downloads/Agent Teams/<english-name>/`；每位員工的 Workflow 位於 `workflows/<workflow-id>/`。
