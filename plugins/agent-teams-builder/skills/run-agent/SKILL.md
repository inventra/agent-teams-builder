...
2. 呼叫 `agent_prepare_run`，讓工具依觸發條件選擇 Skill；若使用者明確指定 Skill，傳入 `skill`。
   - 若使用者指定 Workflow，改呼叫 `workflow_prepare_run`，並依序執行節點。
3. 必須在目前宿主執行：若本 Session 是 Codex，就由 Codex 使用現有工具執行；若本 Session 是 Claude Code，就由 Claude Code 使用現有工具執行。
4. 不得呼叫 Anthropic API、OpenAI API 或另外要求 API Key。對話中的調用由目前 Session 執行；Dashboard 的 Play/排程則只能使用已安裝且已登入的 Codex 或 Claude Code CLI。
